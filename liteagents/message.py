    ...
    def pretty(self) -> (str, any):
        ...
        match self:
            ...
            case ToolMessage(name=name, content=content, role=role):
                content = content.model_dump() if isinstance(content, BaseModel) else content
                # args = ','.join(args.items()) if isinstance(args, dict) else args

                return f"{role.capitalize()} ({name})", Syntax(f'{name}(...) = {content})', lexer="scala",
                                                               theme="ansi-light", word_wrap=True)
...
class AssistantMessage(Message):
    ...
    content: Union[str, ToolRequest] = Field(
...
class ToolMessage(Message):
    ...
    id: str = Field(..., description="A unique identifier for the tool invocation.")
    name: str = Field(..., description="The name of the tool.")
    content: str | list | dict | BaseModel = Field(..., description="The output generated by a tool.")
